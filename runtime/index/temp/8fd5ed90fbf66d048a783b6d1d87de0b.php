...
<!-- conntent start  -->
<div class="am-g my-content user-register-container" <?php if(!empty($user_register_bg_images)): ?>style="background: #FAFAFA url(<?php echo AttachmentPathViewHandle($user_register_bg_images); ?>);"<?php endif; ?>>
    <div class="am-container">
        <!-- 用户注册页面顶部钩子 -->
        <?php if(isset($shopxo_is_develop) and $shopxo_is_develop == true): ?>
            <div class="plugins-tag">
        ...
        <div class="register-container am-u-sm-centered">
            ...
            <div class="register-content">
                <?php if(!empty($home_user_reg_type)): ?>
                    <div data-am-widget="tabs" class="am-tabs am-tabs-d2" data-am-tabs="{noSwipe: 1}">
                        <ul class="am-tabs-nav am-cf">
                            ...
                            <?php endif; if(in_array('sms', $home_user_reg_type)): ?>
                                <li class="<?php if(!in_array('username', $home_user_reg_type)): ?>am-active<?php endif; ?>"><a href="[data-tab-panel-1]">手机注册</a></li>
                            <?php endif; if(in_array('email', $home_user_reg_type)): ?>
                                <li class="<?php if(!in_array('username', $home_user_reg_type) and !in_array('sms', $home_user_reg_type)): ?>am-active<?php endif; ?>"><a href="[data-tab-panel-2]">邮箱注册</a></li>
                            <?php endif; ?>
                        </ul>
                                ...
                                <div data-tab-panel-0 class="am-tab-panel am-active">
                                    ...
                                    <form class="am-form form-validation-username" method="post" action="<?php echo MyUrl('index/user/reg'); ?>" request-type="ajax-fun" request-value="RegisterSuccessBack">
                                        <div class="am-form-group am-form-group-refreshing business-form-block">
                                            <label>用户名</label>
                                            <input type="text" name="accounts" class="am-radius am-form-field" placeholder="请使用字母、数字、下划线 2~18 个字符" pattern="<?php echo MyConst('common_regex_username'); ?>" data-validation-message="用户名格式由 字母数字下划线 2~18 个字符" autocomplete="off" required />
                                        </div>

                                        <div class="am-form-group am-form-group-refreshing business-form-block am-margin-top-xs">
                                            <label>设置登录密码</label>
                                            <div class="am-input-group am-input-group-sm">
                                                <input type="password" name="pwd" class="am-radius am-form-field" placeholder="设置登录密码" pattern="<?php echo MyConst('common_regex_pwd'); ?>" data-validation-message="密码格式 6~18 个字符之间" autocomplete="off" required />
                                                <span class="am-input-group-btn">
                                                ...
                                                <div class="am-input-group am-input-group-sm">
                                                    ...
                                                    <span class="am-input-group-btn">
                                                        <span class="am-btn am-btn-default am-radius form-verify">
                                                            <img src="<?php echo MyUrl('index/user/userverifyentry', ['type'=>'user_reg']); ?>" class="am-radius c-p" id="form-verify-img" onClick="this.src=this.src+((<?php echo htmlentities($url_model); ?> == 0) ? '&' : '?')+Math.random();" />
                                                            <a href="javascript:;" class="form-verify-tips" onClick="document.getElementById('form-verify-img').src='<?php echo MyUrl('index/user/userverifyentry', ['type'=>'user_reg']); if($url_model == 0): ?>&<?php else: ?>?<?php endif; ?>'+Math.random();">更换一张</a>
                                                        </span>
                                                    </span>
                                                </div>
                                            </div>
                                        <?php endif; if(MyC('home_is_enable_userregister_agreement') == 1): ?>
                                            <div class="agreement am-padding-horizontal-sm">
                                                ...
                                                <span>与</span>
                                                <a href="<?php echo MyUrl('index/agreement/index', ['document'=>'userprivacy']); ?>" target="_blank">《隐私政策》</a>
                                            </div>
                                        <?php endif; ?>
    ...
    <div class="am-modal-dialog am-radius">
        ...
        <div class="am-modal-bd">
            <div class="base">
                ...
                <div class="am-fl">
                    <img src="<?php echo MyUrl('index/user/userverifyentry', ['type'=>'user_reg']); ?>" class="am-radius c-p" id="verify-img" onClick="this.src=this.src+((<?php echo htmlentities($url_model); ?> == 0) ? '&' : '?')+Math.random();" />
                    <a href="javascript:;" class="form-verify-tips" onClick="document.getElementById('verify-img').src='<?php echo MyUrl('index/user/userverifyentry', ['type'=>'user_reg']); if($url_model == 0): ?>&<?php else: ?>?<?php endif; ?>'+Math.random();">更换一张</a>
                </div>
            </div>
