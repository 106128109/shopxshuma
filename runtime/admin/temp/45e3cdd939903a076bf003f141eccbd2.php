...
<div class="content-right">
    ...
    <?php if(isset($admin) and isset($admin['id']) and $admin['id'] == 1): ?>
        <!-- 仅超级管理员显示的温馨提示 -->
        <div class="am-alert am-alert-danger" data-am-alert>
            <button type="button" class="am-close">&times;</button>
            <p>1. admin 账户默认拥有所有权限，不可更改。</p>
            <p>2. admin 账户不可更改，但是可以在数据表中修改[ <?php echo MyConfig('database.connections.mysql.prefix'); ?>admin ] 字段 username</p>
        </div>
    <?php endif; ?>
    ...
    <table class="am-table am-table-striped am-table-hover am-table-bordered am-text-nowrap form-table-data-list">
        ...
        <tbody>
            <?php if(!empty($data_list) and !empty($form_table['base']['key_field'])): ?>
                <!-- 处理数据数量小于默认数量 -->
                <?php $__FOR_START_1322016015__=0;$__FOR_END_1322016015__=count($data_list);for($i=$__FOR_START_1322016015__;$i < $__FOR_END_1322016015__;$i+=1){ ?>
                    <!-- 1. tr 主键id名称 -->
                    <!-- 2. 是否黄色选中class -->
                    <tr
